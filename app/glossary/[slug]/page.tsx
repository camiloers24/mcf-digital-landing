...
export function generateMetadata({ params }: Props): Metadata {
  const { frontmatter: fm } = getContentBySlug('glossary', params.slug)
  const title = `${fm.title} | MCF.digital`
  const url = `https://mcf.digital/glossary/${params.slug}/`
...
export default function GlossaryEntryPage({ params }: Props) {
  const { frontmatter: fm, body } = getContentBySlug('glossary', params.slug)
  const canonicalUrl = `https://mcf.digital/glossary/${params.slug}/`
  ...
  if (isTodoAuthor) {
    console.warn(
      `[schema] Skipping Article schema for /glossary/${params.slug}/ — author is a TODO placeholder`
    )
  }
